...
/*
	xor	%eax, %eax	# zero registers, use 32-bit version since it is recommended
	xor	%ebx, %ebx
*/
	xor	%ecx, %ecx
/*
...
//*/
	...
	# write result for this limb, prepare for next limb
	mov	%rbx, 24(%rdi)	# write result for this 64-bit limb
	mov	%rcx, %rbx
	mov	%rsi, %rcx
	xor	%esi, %esi	# zero rsi, use 32-bit version since it is recommended
	...
	# write result for this limb, prepare for next limb
	mov	%rbx, 32(%rdi)	# write result for this 64-bit limb
	mov	%rcx, %rbx
	mov	%rsi, %rcx
	xor	%esi, %esi	# zero rsi, use 32-bit version since it is recommended
	...
	# write result for this limb, prepare for next limb
	mov	%rbx, 40(%rdi)	# write result for this 64-bit limb
	mov	%rcx, %rbx
	mov	%rsi, %rcx
	xor	%esi, %esi	# zero rsi, use 32-bit version since it is recommended
	...
	# write result for this limb, prepare for next limb
	mov	%rbx, 48(%rdi)	# write result for this 64-bit limb
	mov	%rcx, 56(%rdi)	# write result for this 64-bit limb
